...
A `SUBSET` task, which extracts site-wise statistics from the VCF, and partitions them using `subsetVCFstats.pl` based on some input BED, whether one of the classification category BEDs generated by the `CLASSIFY` task, or some other arbitrary BED.

This task will be analogous to how we generated VCF statistic CDFs partitioned by site class (i.e. ER, FN, FP, TN, TP) and developed masking thresholds.
...
Example commands to generate such a MAF file for D. yakuba NY73PB versus D. yakuba Tai18E2:
